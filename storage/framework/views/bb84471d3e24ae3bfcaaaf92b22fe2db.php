...
<style>
    ...
    .filter-bar { background: #f8f9fa; border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; }
    .status-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 5px; }
    .status-dot.active { background: #0ab39c; }
    .status-dot.inactive { background: #adb5bd; }
    .conv-row:hover { background: rgba(64,81,137,0.04) !important; cursor: default; }
    .msg-bubble { background: #f1f3f9; border-radius: 8px; padding: 2px 10px; font-size: 12px; font-weight: 600; color: #405189; }
    .detail-label { font-size: 11px; text-transform: uppercase; color: #adb5bd; font-weight: 700; letter-spacing: 0.5px; }
    .detail-value { font-size: 14px; font-weight: 600; color: #343a40; }
    .modal-header-premium { background: linear-gradient(135deg, #405189, #0ab39c); color: #fff; border-radius: 12px 12px 0 0; }
    .modal-header-premium .btn-close { filter: invert(1); }
    .quick-stat-mini { border-radius: 10px; padding: 10px 14px; flex: 1; text-align: center; }
</style>
<?php $__env->stopPush(); ?>
    ...
    <div class="row mb-4">
            ...
            <div class="card border-0 shadow-sm" style="border-left: 4px solid #405189 !important;">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <h4 class="mb-0 fw-black text-primary"><?php echo e($conversations->total()); ?></h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-3 col-md-6 mb-3 mb-xl-0">
            <div class="card border-0 shadow-sm" style="border-left: 4px solid #adb5bd !important;">
                <div class="card-body d-flex align-items-center gap-3">
                    <div class="avatar-sm bg-soft-secondary rounded-circle d-flex align-items-center justify-content-center">
            ...
            <div class="card border-0 shadow-sm" style="border-left: 4px solid #f7b84b !important;">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <h4 class="mb-0 fw-black text-warning"><?php echo e($conversations->count()); ?></h4>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-header border-bottom-dashed d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <div class="input-group" style="width: 220px;">
                            ...
                            <input type="text" id="convSearch" class="form-control border-0 bg-light" placeholder="Search user or product...">
                        </div>
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0" id="convTable">
                            ...
                            <tbody id="convBody">
                                <?php $__currentLoopData = $conversations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $conv): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="conv-row border-bottom"
                                    data-status="<?php echo e($conv->status); ?>"
                                    data-search="<?php echo e(strtolower(($conv->userOne->name ?? '') . ' ' . ($conv->userTwo->name ?? '') . ' ' . ($conv->product->title ?? ''))); ?>">
                                    <td class="ps-4 text-muted fw-bold fs-12">
                                        <?php echo e(($conversations->currentPage() - 1) * $conversations->perPage() + $index + 1); ?>
                                    ...
                                    <td>
                                        <?php if($conv->product): ?>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div>
                                                    <p class="mb-0 fw-semibold fs-13 text-truncate" style="max-width:130px;"><?php echo e($conv->product->title); ?></p>
                                                    <small class="text-muted">ID #<?php echo e($conv->product_id); ?></small>
                                                </div>
                                            </div>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="conv-avatar bg-soft-info text-info" style="background: #cff4fc;">
                                                <?php echo e(strtoupper(substr($conv->userOne->name ?? 'U', 0, 1))); ?>

                                            </div>
                                            <div>
                                                <p class="mb-0 fw-semibold fs-13"><?php echo e($conv->userOne->name ?? 'N/A'); ?></p>
                                                <small class="text-muted"><?php echo e($conv->userOne->email ?? ''); ?></small>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="conv-avatar bg-soft-warning text-warning" style="background: #fff3cd;">
                                                <?php echo e(strtoupper(substr($conv->userTwo->name ?? 'U', 0, 1))); ?>

                                            </div>
                                            <div>
                                                <p class="mb-0 fw-semibold fs-13"><?php echo e($conv->userTwo->name ?? 'N/A'); ?></p>
                                                <small class="text-muted"><?php echo e($conv->userTwo->email ?? ''); ?></small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <span class="msg-bubble"><?php echo e($conv->messages_count); ?></span>
                                    </td>
                                    <td>
                                        <span class="d-flex align-items-center">
                                            <span class="status-dot <?php echo e($conv->status == 'active' ? 'active' : 'inactive'); ?>"></span>
                                            <span class="badge <?php echo e($conv->status == 'active' ? 'bg-soft-success text-success' : 'bg-soft-secondary text-secondary'); ?> fs-11 fw-bold">
                                                <?php echo e(ucfirst($conv->status ?? 'N/A')); ?>

                                            </span>
                                        </span>
                                    </td>
                                    <td>
                                        <p class="mb-0 fs-13"><?php echo e($conv->created_at->format('d M Y')); ?></p>
                                        <small class="text-muted"><?php echo e($conv->created_at->format('h:i A')); ?></small>
                                    </td>
                                    <td class="text-center">
                                        <button class="btn btn-soft-primary btn-sm rounded-pill px-3"
                                            data-bs-toggle="modal"
                                            data-bs-target="#convDetailModal"
                                            data-id="<?php echo e($conv->id); ?>"
                                            data-product="<?php echo e($conv->product->title ?? 'N/A'); ?>"
                                            data-product-id="<?php echo e($conv->product_id ?? 'N/A'); ?>"
                                            data-user-one="<?php echo e($conv->userOne->name ?? 'N/A'); ?>"
                                            data-user-one-email="<?php echo e($conv->userOne->email ?? ''); ?>"
                                            data-user-two="<?php echo e($conv->userTwo->name ?? 'N/A'); ?>"
                                            data-user-two-email="<?php echo e($conv->userTwo->email ?? ''); ?>"
                                            data-messages="<?php echo e($conv->messages_count); ?>"
                                            data-status="<?php echo e($conv->status); ?>"
                                            data-created="<?php echo e($conv->created_at->format('d M Y, h:i A')); ?>">
        ...
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 14px; overflow: hidden;">
                <div class="modal-header modal-header-premium py-3 px-4">
                    <div>
                        <h5 class="modal-title mb-0 fw-bold text-white"><i class="ri-message-3-fill me-2"></i>Conversation Detail</h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('convDetailModal').addEventListener('show.bs.modal', function (e) {
        ...
        document.getElementById('modal-product-id').textContent = 'Product ID: ' + btn.dataset.productId;
        ...
        statusBadge.textContent = btn.dataset.status ? btn.dataset.status.charAt(0).toUpperCase() + btn.dataset.status.slice(1) : 'N/A';
        statusBadge.className = 'badge fs-12 fw-bold mt-1 ' + (isActive ? 'bg-success' : 'bg-secondary');
    });
});
</script>
<?php $__env->stopPush(); ?>
