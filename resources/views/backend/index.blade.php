...
@section('content')
    ...
    <div class="dash-page">
        ...
        <div class="dash-header">
            <div>
                <h4 class="dash-welcome">Welcome back, {{ Auth::user()->name }}! 👋</h4>
                <p class="dash-welcome-sub">Here is the latest snapshot of your platform performance.</p>
            </div>
            <a href="{{ route('backend.products.create') }}" class="btn-add-product">
        ...
        {{-- Stat Cards Row 1 — colored --}}
        <div class="stat-grid-4">
            <div class="stat-card sc-blue">
                <div class="sc-label">Total Users</div>
                <div class="sc-val">{{ number_format($totalUsers) }}</div>
                <div class="sc-sub">{{ $verifiedUsers }} Verified</div>
                <div class="sc-icon">
            ...
            <div class="stat-card sc-green">
                ...
                <div class="sc-val">${{ number_format($totalRevenue, 2) }}</div>
            ...
            <div class="stat-card sc-cyan">
                ...
                <div class="sc-val">{{ number_format($totalProducts) }}</div>
                <div class="sc-sub">{{ $spotlightedProducts }} Spotlighted</div>
                <div class="sc-icon">
                    <svg width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.8"
                        viewBox="0 0 24 24">
                        <path
            ...
            <div class="stat-card sc-amber">
                ...
                <div class="sc-val">{{ number_format($totalGarageSales) }}</div>
        ...
        {{-- Stat Cards Row 2 — neutral --}}
        <div class="stat-grid-4">
            <div class="stat-card sc-neutral">
                <div class="sc-label">Total Matches</div>
                <div class="sc-val">{{ number_format($totalMatches) }}</div>
            ...
            <div class="stat-card sc-neutral">
                ...
                <div class="sc-val sc-val-green">{{ $matchConversionRate }}%</div>
                <div class="sc-sub">{{ $completedPickups }} Completed Pickups</div>
            ...
            <div class="stat-card sc-neutral">
                ...
                <div class="sc-val">{{ number_format($totalMessages) }}</div>
            ...
            <div class="stat-card sc-neutral">
                ...
                <div class="sc-val sc-val-red">{{ number_format($totalReports) }}</div>
        ...
        <div class="chart-row-main">
            ...
            <div class="chart-card">
                ...
                <div class="perf-summary">
                    <div class="ps-item">
                        <div class="ps-val" id="ps-orders">{{ $totalOrders }}</div>
                        <div class="ps-label">Pickup Requests</div>
                    </div>
                    <div class="ps-item">
                        <div class="ps-val" id="ps-rev">${{ number_format($totalRevenue, 0) }}</div>
                        <div class="ps-label">Total Revenue</div>
                    </div>
                    <div class="ps-item">
                        <div class="ps-val" id="ps-msg">{{ $totalMessages }}</div>
                        <div class="ps-label">Messages</div>
                    </div>
                    <div class="ps-item">
                        <div class="ps-val ps-green">{{ $matchConversionRate }}%</div>
            ...
            <div class="chart-card">
                ...
                <div class="rev-breakdown">
                    <div class="rb-row rb-border">
                        ...
                        <span class="rb-val">${{ number_format($revenueSources['Spotlight'], 2) }}</span>
                    </div>
                    <div class="rb-row">
                        <span class="cl-item"><span class="cl-dot" style="background:#1d9e75"></span>Garage Sales</span>
                        <span class="rb-val">${{ number_format($revenueSources['Garage Sales'], 2) }}</span>
                    </div>
                </div>
            </div>
        ...
        <div class="chart-row-3">
            ...
            <div class="chart-card">
                ...
                <div class="chart-legend" style="justify-content:center;margin-top:10px;">
                    @foreach ($pickupStatusDistribution as $ps)
                        <span class="cl-item">
                            <span class="cl-dot"
                                style="background:{{ ['completed' => '#1d9e75', 'pending' => '#378add', 'scheduled' => '#ba7517', 'cancelled' => '#d85a30'][$ps->status] ?? '#888' }}"></span>
                            {{ ucfirst($ps->status) }} ({{ $ps->total }})
                        </span>
                    @endforeach
                </div>
            </div>
            ...
            <div class="chart-card">
                ...
                <div class="chart-wrap" style="height:{{ count($categoryDistribution) * 38 + 60 }}px; min-height:200px;">
            ...
            <div class="chart-card">
                ...
                <div class="activity-feed">
                    @foreach ($recentActivities as $act)
                        <div class="act-item">
                            <div class="act-dot act-{{ strtolower($act['type']) }}">
                                <i class="{{ $act['icon'] }}"></i>
                            </div>
                            <div class="act-body">
                                <div class="act-title">{{ $act['title'] }}</div>
                                <div class="act-meta">
                                    <span class="act-badge act-badge-{{ $act['color'] }}">{{ $act['type'] }}</span>
                                    <span>{{ $act['time']?->diffForHumans() ?? '—' }}</span>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const ALL = {
                ...
                orders: @json($orders),
                messages: @json($messageActivity),
            };

            const REVENUE_SOURCES = @json(array_values($revenueSources));
            const REVENUE_LABELS = @json(array_keys($revenueSources));
            const PICKUP_DATA = @json($pickupStatusDistribution);
            const CATEGORY_DATA = @json($categoryDistribution);
