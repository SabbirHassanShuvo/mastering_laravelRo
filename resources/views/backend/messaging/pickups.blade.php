...
@push('styles-bottom')
    <style>
        ...
        .row-hover:hover {
            background: rgba(10, 179, 156, 0.04) !important;
        }
    ...
    <div class="row mb-4">
        @php
            $statMap = [
                ...
                'accepted' => ['color' => 'success', 'icon' => 'ri-checkbox-circle-line'],
                'rejected' => ['color' => 'danger', 'icon' => 'ri-close-circle-line'],
                'completed' => ['color' => 'info', 'icon' => 'ri-check-double-line'],
            ];
        @endphp
        @foreach ($statMap as $status => $cfg)
            <div class="col-xl-3 col-md-6 mb-3 mb-xl-0">
                <div class="card border-0 shadow-sm"
                    style="border-left: 4px solid var(--vz-{{ $cfg['color'] }}) !important;">
                    <div class="card-body d-flex align-items-center gap-3">
                        <div
                            class="avatar-sm bg-soft-{{ $cfg['color'] }} rounded-circle d-flex align-items-center justify-content-center">
                            <i class="{{ $cfg['icon'] }} fs-22 text-{{ $cfg['color'] }}"></i>
                        </div>
                        <div>
                            <p class="text-muted mb-0 fs-12 fw-bold text-uppercase">{{ ucfirst($status) }}</p>
                            <h4 class="mb-0 fw-black text-{{ $cfg['color'] }}">{{ $pickupStatusCounts[$status] ?? 0 }}</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-header border-bottom-dashed d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <div class="input-group" style="width: 230px;">
                            ...
                            <input type="text" id="pickupSearch" class="form-control border-0 bg-light"
                        ...
                        <table class="table table-hover align-middle mb-0" id="pickupTable">
                            ...
                            <tbody id="pickupBody">
                                @foreach ($pickups as $pickup)
                                    ...
                                    @endphp
                                    <tr class="row-hover border-bottom" data-status="{{ $pickup->status }}"
                                        data-search="{{ strtolower(($pickup->product->title ?? '') . ' ' . ($pickup->requester->name ?? '') . ' ' . ($pickup->location ?? '')) }}">
                                        <td class="ps-4 text-muted fw-bold fs-12">
                                            {{ ($pickups->currentPage() - 1) * $pickups->perPage() + $loop->index + 1 }}
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div>
                                                    <p class="mb-0 fw-semibold fs-13 text-truncate"
                                                        style="max-width:130px;">{{ $pickup->product->title ?? 'N/A' }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="pickup-avatar bg-soft-success text-success">
                                                    {{ strtoupper(substr($pickup->requester->name ?? 'U', 0, 1)) }}
                                                </div>
                                                <div>
                                                    <p class="mb-0 fw-semibold fs-13">
                                                        {{ $pickup->requester->name ?? 'N/A' }}</p>
                                                    <small class="text-muted">{{ $pickup->requester->email ?? '' }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <p class="mb-0 fw-semibold fs-13"><i
                                                    class="ri-calendar-line me-1 text-primary"></i>{{ $pickup->pickup_date ?? 'N/A' }}
                                            </p>
                                            <small class="text-muted"><i
                                                    class="ri-time-line me-1"></i>{{ $pickup->pickup_time ?? '—' }}</small>
                                        </td>
                                        <td>
                                            <span class="fs-12 text-muted" title="{{ $pickup->location }}">
                                                <i
                                                    class="ri-map-pin-line me-1 text-danger"></i>{{ Str::limit($pickup->location ?? '—', 28) }}
                                            </span>
                                        </td>
                                        <td><span class="{{ $badgeClass }}">{{ ucfirst($pickup->status) }}</span></td>
                                        <td>
                                            <p class="mb-0 fs-13">{{ $pickup->created_at->format('d M Y') }}</p>
                                            <small class="text-muted">{{ $pickup->created_at->format('h:i A') }}</small>
                                        </td>
                                        <td class="text-center">
                                            <button class="btn btn-soft-success btn-sm rounded-pill px-3"
                                                data-bs-toggle="modal" data-bs-target="#pickupDetailModal"
                                                data-id="{{ $pickup->id }}"
                                                data-product="{{ $pickup->product->title ?? 'N/A' }}"
                                                data-requester="{{ $pickup->requester->name ?? 'N/A' }}"
                                                data-requester-email="{{ $pickup->requester->email ?? '' }}"
                                                data-receiver="{{ $pickup->receiver->name ?? 'N/A' }}"
                                                data-receiver-email="{{ $pickup->receiver->email ?? '' }}"
                                                data-date="{{ $pickup->pickup_date ?? 'N/A' }}"
                                                data-time="{{ $pickup->pickup_time ?? 'N/A' }}"
                                                data-location="{{ $pickup->location ?? 'N/A' }}"
                                                data-status="{{ $pickup->status }}" data-badge-class="{{ $badgeClass }}"
                                                data-created="{{ $pickup->created_at->format('d M Y, h:i A') }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('pickupDetailModal').addEventListener('show.bs.modal', function(e) {
                ...
                statusEl.innerHTML = '<span class="' + btn.dataset.badgeClass + '">' + btn.dataset.status
                    .charAt(0).toUpperCase() + btn.dataset.status.slice(1) + '</span>';
            });
        });
    </script>
@endpush
