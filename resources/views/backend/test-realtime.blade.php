...
@section('title', 'Reverb Diagnostics & Chat Test')
                                ...
                                <select class="form-select mb-3" id="conversationSelect">
                                    ...
                                    @foreach($conversations as $conv)
                                        <option value="{{ $conv->id }}">
                                            ID: {{ $conv->id }} | {{ $conv->userOne->name }} & {{ $conv->userTwo->name }} 
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function sendMessage() {
            ...
            const url = currentMode === 'public' 
                ? "{{ route('backend.trigger_test_event') }}" 
                : "{{ route('backend.trigger_private_event') }}";
