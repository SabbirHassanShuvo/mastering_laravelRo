...
@section('title', 'City Market Explorer')
    ...
    <div class="row align-items-center mb-5 pop-in">
        ...
        <div class="col-auto">
            <div class="d-flex gap-2">
                ...
                <a href="{{ route('backend.spotlight.index') }}" class="btn btn-primary rounded-pill shadow-primary px-4">
    ...
    <div class="row g-4 mb-5">
        <div class="col-xl-3 col-md-6 pop-in" style="animation-delay: 0.1s;">
            <div class="card shadow-sm p-4 h-100 bg-soft-primary border-start border-primary border-4">
                ...
                <h2 class="fw-black text-primary mb-1 display-5">{{ $cityRevenue->count() }}</h2>
        ...
        <div class="col-xl-3 col-md-6 pop-in" style="animation-delay: 0.2s;">
            <div class="card shadow-sm p-4 h-100 bg-soft-success border-start border-success border-4">
                ...
                <h2 class="fw-black text-success mb-1 display-5">${{ number_format($cityRevenue->max('revenue'), 0) }}</h2>
                <p class="text-dark fw-bold mb-0 mt-2 fs-14">
                    <i class="ri-building-2-fill me-1"></i> {{ $cityRevenue->first()->city ?? 'None' }}
                </p>
            </div>
        </div>
        <div class="col-xl-3 col-md-6 pop-in" style="animation-delay: 0.3s;">
            <div class="card shadow-sm p-4 h-100 bg-soft-info border-start border-info border-4">
                ...
                <h2 class="fw-black text-info mb-1 display-5">{{ number_format($cityRevenue->avg('capture_rate'), 1) }}%</h2>
        ...
        <div class="col-xl-3 col-md-6 pop-in" style="animation-delay: 0.4s;">
            <div class="card shadow-sm p-4 h-100 bg-soft-warning border-start border-warning border-4">
                ...
                <h2 class="fw-black text-warning mb-1 display-5">{{ number_format($cityRevenue->max('capture_rate'), 1) }}%</h2>
                <p class="text-dark fw-bold mb-0 mt-2 fs-14">
                    <i class="ri-flashlight-line me-1"></i> {{ $cityRevenue->sortByDesc('capture_rate')->first()->city ?? 'None' }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="market-hub shadow-sm mb-5 pop-in" style="animation-delay: 0.5s;">
        <div class="d-flex align-items-center mb-4">
            ...
            <div class="text-muted fs-12 fw-bold bg-light px-3 py-1 rounded-pill">
                Showing {{ $cityRevenue->count() }} Market Entities
            </div>
        </div>

        <div class="market-grid-container">
            <div class="row g-4" id="marketTiles">
                @foreach($cityRevenue as $idx => $city)
                @php 
                    $rank = $idx + 1;
                ...
                @endphp
                <div class="col-xl-4 col-lg-6 market-tile" data-name="{{ strtolower($city->city) }}">
                    <div class="city-card-detailed p-4">
                        ...
                        <div class="d-flex align-items-start mb-2">
                            ...
                            <div>
                                <h3 class="city-name-large mb-0">{{ $city->city }}</h3>
                        ...
                        <div class="revenue-display">
                            ...
                            <span class="revenue-amount">${{ number_format($city->revenue, 2) }}</span>
                            <span class="share-badge">{{ number_format($city->contribution, 1) }}% Global Share</span>
                        </div>

                        <div class="metrics-grid">
                            <div class="metric-row">
                                <span class="metric-label"><i class="ri-user-star-fill me-1"></i> Unique Boosters</span>
                                <span class="metric-value">{{ $city->unique_users }}</span>
                            </div>
                            <div class="metric-row">
                                <span class="metric-label"><i class="ri-flashlight-fill me-1"></i> Paid Boosts</span>
                                <span class="metric-value">{{ $city->boost_count }}</span>
                            </div>
                            <div class="metric-row">
                                <span class="metric-label"><i class="ri-stack-line me-1"></i> Total Inventory</span>
                                <span class="metric-value">{{ $city->total_products }}</span>
                            </div>
                            <div class="metric-row">
                                <span class="metric-label"><i class="ri-scales-3-line me-1"></i> Avg. Boost Value</span>
                                <span class="metric-value">${{ number_format($city->avg_boost_value, 1) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            <div class="d-flex justify-content-between align-items-end mb-1">
                                ...
                                <span class="fw-black fs-14" style="color: {{ $progressColor }}">{{ number_format($city->capture_rate, 1) }}%</span>
                            </div>
                            <div class="capture-track">
                                <div class="capture-fill" style="width: {{ $city->capture_rate }}%; background: {{ $progressColor }}; box-shadow: 0 0 8px {{ $progressColor }}40;"></div>
                            </div>
                        </div>

                        <div class="mt-4 pt-3 border-top d-flex justify-content-between align-items-center">
                            <span class="text-muted fs-11"><i class="ri-time-line me-1"></i>{{ $city->last_boost_at ? \Carbon\Carbon::parse($city->last_boost_at)->diffForHumans() : 'No activity' }}</span>
